...
namespace BookShoppingCartMvcUI.Repositories
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<ShoppingCarts> GetUserCart()
        {
            var userId = GetUserId();
            if (userId == null)
                throw new Exception("Invalid userid");
            var MiniShoppingSystem = await _db.ShoppingCarts
                                  .Include(a => a.CartDetails)
                                  .ThenInclude(a => a.Book)
                                  .ThenInclude(a => a.Genre)
                                  .Where(a => a.UserId == userId).FirstOrDefaultAsync();
            return MiniShoppingSystem;

        }
        public async Task<ShoppingCarts> GetCart(string userId)
        {
        ...
        public async Task<bool> DoCheckout()
        {
            ...
            try
            {
                ...
                {
                    var orderDetail = CreateOrderDetail(order.Id, item);
                    AddOrderDetail(orderDetail);
                }

                RemoveCartDetails(cartDetail);

                //await _db.SaveChangesAsync(); // Commit transaction after all changes are made
        ...
        private OrderDetail CreateOrderDetail(int orderId, CartDetail cartDetail)
        {
        ...
        private void RemoveCartDetails(List<CartDetail> cartDetails)
        {
